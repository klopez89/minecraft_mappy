...
function continueToLoadMapper() {
	if (hasLatestMapImgUrlExpired()) {
		...
		// 'latest_backup_id': localStorage.getItem('latest_backup_id')
		requestData = {
	...
	$.ajax({
		...
		success: function(response) {
	      ...
	      presentMapExplorer(signed_img_url);
			...
			const latest_map_img_url = response["latest_map_img_url"];
			const latest_blob_path = response["latest_blob_path"];

			if (latest_blob_path != null) {
				console.log(`We have a newer map to load into! Show the user this option as a button! New blob_path: ${latest_blob_path}, and new img url: ${latest_map_img_url}`)
			}
